...
from rchat.schemas.models import Session, ChatTypeEnum
from rchat.state import app_state
from rchat.views.auth.helpers import check_access_token
from rchat.views.chat.helpers import get_chat_name
from rchat.views.chat.models import ChatListResponse, ChatListItem, LastChatMessage
...
async def get_chat_list(session: Session = Depends(check_access_token)):
    ...
    for chat in user_chats:
        last_chat_message = (
            await app_state.message_repo.get_last_chat_message(chat_id=chat.id)
        )
        ...
        chat_list.append(
            ChatListItem(
                ...
                ) if last_chat_message else None,
